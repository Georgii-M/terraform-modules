...
output "id" {
  ...
  value       = element(concat(aws_instance.this.*.id, aws_nat_gateway.this.*.id, [""]), 0)
}
...
output "primary_network_interface_id" {
  ...
  value       = element(concat(aws_instance.this.*.primary_network_interface_id, aws_nat_gateway.this.*.network_interface_id, [""]), 0)
}
...
output "public_ip" {
  ...
  value       = element(concat(aws_instance.this.*.public_ip, aws_nat_gateway.this.*.public_ip, [""]), 0)
}

output "tags_all" {
  description = "A map of tags assigned to the resource, including those inherited from the provider default_tags configuration block"
  value       = element(concat(aws_instance.this.*.tags_all, aws_nat_gateway.this.*.tags_all, [""]), 0)
}